...
class CornersProblem(search.SearchProblem):
  ...
  def __init__(self, startingGameState):
    ...
    "*** YOUR CODE HERE ***"
    cornerVisit = [False, False, False, False]
    index = 0
    for corner in self.corners:
      if self.startingPosition == corner:
        cornerVisit[i] = True
      index += 1
    self.startingCorners = tuple(cornerVisit) 
  ...
  def getSuccessors(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      if not hitsWall:
        cornerVisit = state[1]
        cornerVisit = list(cornerVisit)
        index = 0
        for corner in self.corners:
          if (nextx, nexty) == corner:
            cornerVisit[index] = True
          index += 1
        cornerVisit = tuple(cornerVisit)
        successors.append((((nextx, nexty), cornerVisit), action, 1) )
    self._expanded += 1
    return successors
