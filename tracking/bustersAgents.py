...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    positions = []
    for i in range(len(gameState.getLivingGhosts())):
        if gameState.getLivingGhosts()[i]:
            max, max_pos = 0, (1,1)
            for pos in self.ghostBeliefs[i-1]:
                val = self.ghostBeliefs[i-1][pos]    
                if val > max:
                    max = val
                    max_pos = pos
            positions.append(max_pos)
    min, target = 2000000000, None
    for pos in positions:
        d = self.distancer.getDistance(pacmanPosition, pos)
        if d < min:
            min = d
            target = pos
    min, a = 2000000000, None
    for action in legal:
        pos = Actions.getSuccessor(pacmanPosition, action)
        d = self.distancer.getDistance(pos, target)
        if d < min:
            min = d
            a = action
    return a
